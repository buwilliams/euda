...
class TestCrossPollination:
    """Test cross-pollination of memories from chat to user."""
    ...
    def test_cross_pollinate_copies_user_relevant_types(self, tmp_path):
        ...
        try:
            ...
            user_dir = tmp_path / "agents" / "user" / "memory"
            user_dir.mkdir(parents=True, exist_ok=True)

            items = [
                {"id": "mem-1", "type": "person", "short_description": "Alice"},
                {"id": "mem-2", "type": "learning", "short_description": "How to code"},
                {"id": "mem-3", "type": "goal", "short_description": "Learn Python"}
            ]

            today = datetime.now().strftime("%Y-%m-%d")
            count = _cross_pollinate_to_user(items, today)

            # Only person and goal should be copied (not learning)
            assert count == 2
        finally:
            for p in patches:
                p.stop()
    ...
    def test_cross_pollinate_avoids_duplicates(self, tmp_path):
        ...
        try:
            ...
            _save_entries([
                {"id": "existing", "type": "person", "short_description": "Alice"}
            ], "user")

            items = [
                {"id": "mem-1", "type": "person", "short_description": "Alice"},  # Duplicate
                {"id": "mem-2", "type": "person", "short_description": "Bob"}  # New
            ]
