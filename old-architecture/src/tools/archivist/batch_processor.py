"""
Batch processor for archivist - processes multiple files in a single API call.
...
AGENTS_DIR = DATA_DIR / "shared" / "state" / "agents"


def load_batch_system_prompt() -> str:
    """
    Load the archivist agent files for use as batch system prompt.

    Combines core agent + archivist persona + batch-specific instructions.
    """
    # Load agent files
    core_file = AGENTS_DIR / "0_core.agent.md"
    persona_file = AGENTS_DIR / "1_archivist.agent.md"
...
def process_batch_with_provider(
    ...
    Args:
        ...
        system_prompt: Optional system prompt (defaults to archivist identity)
    ...
    # Use archivist identity as system prompt if not provided
