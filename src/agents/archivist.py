...
from ..tools.archivist.files import FILE_TOOLS, FILE_HANDLERS, list_pending_files
from ..tools.archivist.token_budget import get_budget, TOKEN_BUDGET_TOOLS, TOKEN_BUDGET_HANDLERS
from ..tools.archivist.classifier import CLASSIFIER_TOOLS, CLASSIFIER_HANDLERS, mark_as_processed
from ..tools.archivist.digest import generate_digest, get_content_for_ai, DIGEST_TOOLS, DIGEST_HANDLERS
from ..tools.archivist.queue import get_queue, QUEUE_TOOLS, QUEUE_HANDLERS


# Combined tools and handlers for the archivist agent
ALL_TOOLS = (
    LOG_TOOLS +
    FILE_TOOLS +
    TOKEN_BUDGET_TOOLS +
...
# Inbox paths - now under archivist agent directory
DATA_DIR = Path(__file__).parent.parent.parent / "data"
ARCHIVIST_DIR = DATA_DIR / "archivist"
INBOX_DIR = ARCHIVIST_DIR / "state" / "inbox"
PENDING_DIR = INBOX_DIR / "pending"
PROCESSING_DIR = INBOX_DIR / "processing"


def create_archivist_agent(include_file_tools: bool = True):
    """
    Create an Archivist Agent instance.
...
def run_interactive():
    ...
    agent = create_archivist_agent(include_file_tools=False)
...
class AutonomousArchivistAgent(AutonomousAgent):
    """
...
def get_archivist_status() -> str:
    """Get current archivist status for display."""
    queue = get_queue()
    budget = get_budget()
