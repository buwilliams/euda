"""
Skill Context - Build environment variables for skill execution.

Skills receive context through environment variables, keeping them decoupled
...
def get_data_dir_from_env() -> Path:
    """Get the data directory from environment (for use inside skills).
...
def get_agent_id_from_env() -> Optional[str]:
    """Get the agent ID from environment (for use inside skills)."""
    return os.environ.get("EUNO_AGENT_ID")


def get_topic_id_from_env() -> Optional[str]:
    """Get the topic ID from environment (for use inside skills)."""
    return os.environ.get("EUNO_TOPIC_ID")


def get_session_id_from_env() -> Optional[str]:
    """Get the session ID from environment (for use inside skills)."""
    return os.environ.get("EUNO_SESSION_ID")


def get_cwd_from_env() -> Path:
    """Get the original working directory from environment (for use inside skills).

    Skills run with cwd set to the skill directory, but file path arguments
