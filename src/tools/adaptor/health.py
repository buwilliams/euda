...
PROFILER_DIR = DATA_DIR / "profiler"
...
def assess_configuration() -> dict:
    ...
    # Check curator config
    curator_config = DATA_DIR / "curator" / "config"
    if curator_config.exists():
        config_files = list(curator_config.glob("*.json"))
        if config_files:
    ...
    bio_file = PROFILER_DIR / "state" / "context" / "biographical.md"
    if bio_file.exists():
...
def assess_agent_activity() -> dict:
    ...
    agents = ["archivist", "profiler", "curator", "friend", "worker", "adaptor"]
    ...
    # Special check for archivist - check queue
    pending_dir = DATA_DIR / "archivist" / "state" / "inbox" / "pending"
    if pending_dir.exists():
        pending_count = len([f for f in pending_dir.iterdir() if f.is_file() and not f.name.startswith('.')])
        results["archivist"]["pending_files"] = pending_count
        if pending_count > 0:
            results["archivist"]["active"] = True
...
def get_progress_metrics() -> dict:
    ...
    processed_dir = DATA_DIR / "archivist" / "state" / "inbox" / "processed"
    ...
    # Count values
    values_dir = PROFILER_DIR / "state" / "values"
    ...
    # Count patterns
    patterns_file = PROFILER_DIR / "state" / "behaviors" / "patterns.md"
    if patterns_file.exists():
        content = patterns_file.read_text()
        metrics["patterns_identified"] = content.count("## ") - 1  # Subtract header

    # Count opportunities (now managed by curator)
    opps_file = DATA_DIR / "curator" / "state" / "opportunities" / "opportunities.json"
    if opps_file.exists():
        try:
...
def generate_agent_guidance(gaps: list) -> dict:
    ...
    # Curator guidance (was attention)
    curator_guidance = {}
    if "config.energy_baseline" in gap_ids:
        curator_guidance["ask_energy_in_morning"] = True
        curator_guidance["energy_reason"] = "No energy baseline established yet"
    guidance["guidance"]["curator"] = curator_guidance

    # Friend guidance (was interaction)
    friend_guidance = {}
    if "biographical.name" in gap_ids:
        friend_guidance["learn_name_naturally"] = True
        friend_guidance["name_reason"] = "User's name not yet known"
    if "biographical.location" in gap_ids:
        friend_guidance["learn_location_naturally"] = True
        friend_guidance["location_reason"] = "User location unknown"
    guidance["guidance"]["friend"] = friend_guidance

    # Profiler guidance (was synthesis)
    profiler_guidance = {}
    if any("biographical" in g for g in gap_ids):
        profiler_guidance["prioritize_biographical"] = True
        profiler_guidance["biographical_reason"] = "Context files need population"
    guidance["guidance"]["profiler"] = profiler_guidance
