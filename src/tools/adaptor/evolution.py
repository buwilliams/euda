...
EVOLUTION_DIR = ADAPTOR_OUTPUT_DIR
INTROSPECTION_DIR = ADAPTOR_OUTPUT_DIR


def list_agents() -> str:
    """
    List all agents in the system by scanning agent files.
...
def get_agent_identity(agent_name: str) -> str:
    """
    Read the full agent file for an agent.

    Args:
        agent_name: The agent name (e.g., 'archivist', 'profiler')
...
def analyze_agent_code(agent_name: str) -> str:
    ...
    """
    agent_file = AGENTS_CODE_DIR / f"{agent_name}.py"
...
def get_system_overview() -> str:
    ...
    agent_code_files = list(AGENTS_CODE_DIR.glob("*.py"))
    agent_code_count = len([f for f in agent_code_files if not f.name.startswith("__")])
...
## Key Directories
- `data/shared/state/agents/` - Agent personas and beliefs
- `src/agents/` - Agent implementations
- `src/tools/` - Tool definitions and handlers
- `data/shared/state/lifelog/` - Life log entries
- `data/shared/state/profile/` - User profile
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                "agent_name": {
                    "type": "string",
                    "description": "The agent name (e.g., 'archivist', 'profiler', 'worker')"
                }
            },
            "required": ["agent_name"]
        }
...
# Import archivist status tools
from ..archivist.status import ARCHIVIST_STATUS_TOOLS, ARCHIVIST_STATUS_HANDLERS

# Combined tools and handlers (evolution + agent logs + archivist status)
EVOLUTION_TOOLS = _BASE_EVOLUTION_TOOLS + AGENT_LOG_TOOLS + ARCHIVIST_STATUS_TOOLS
EVOLUTION_HANDLERS = {**_EVOLUTION_HANDLERS, **AGENT_LOG_HANDLERS, **ARCHIVIST_STATUS_HANDLERS}
