...
class TopicsSyncHandler(SyncHandler):
    ...
    def detect_changes(
        ...
        # Load tombstones (local + remote)
        local_deletions = get_deletions()
        remote_deletions = self._get_remote_deletions(transport)
        tombstones = self._merge_topic_tombstones(local_deletions, remote_deletions)
        ...
        # Filter out tombstoned topics/logs to avoid reintroducing deletions
        if tombstones:
            local_topics = {k: v for k, v in local_topics.items() if k not in tombstones}
            remote_topics = {k: v for k, v in remote_topics.items() if k not in tombstones}
            local_logs = [l for l in local_logs if l.get("topic_id") not in tombstones]
            remote_logs = [l for l in remote_logs if l.get("topic_id") not in tombstones]
    ...
    def apply_changes(
        ...
        """Apply topic changes."""
        from src.core.data.topics import export_topics, import_topics, delete_topic
        from src.core.data.deletions import get_deletions
        ...
        # Apply tombstones before any import/export
        deletions = get_deletions()
        tombstones = deletions.get("topics", {})
        if tombstones:
            for topic_id, meta in tombstones.items():
                delete_children = False
                if isinstance(meta, dict):
                    delete_children = bool(meta.get("delete_children", False))
                delete_topic(topic_id, delete_children=delete_children)
            if direction in ("push", "bidirectional"):
                self._apply_remote_topic_deletions(transport, tombstones)
        ...
        if has_push:
            ...
            if tombstones:
                local_data["topics"] = [
                    t for t in local_data["topics"] if t.get("id") not in tombstones
                ]
                local_data["logs"] = [
                    l for l in local_data.get("logs", []) if l.get("topic_id") not in tombstones
                ]
    ...
    def _merge_topic_tombstones(self, local: dict, remote: dict) -> Dict[str, dict | str]:
        merged: Dict[str, dict | str] = {}
        for source in (local, remote):
            topics = source.get("topics", {})
            if isinstance(topics, dict):
                for topic_id, meta in topics.items():
                    merged[topic_id] = meta
        return merged
    ...
    def _apply_remote_topic_deletions(self, transport: Transport, tombstones: dict) -> None:
        if not tombstones:
            return
        import tempfile
        import subprocess
        ...
        remote_temp = "/tmp/euno_topic_deletions.json"
        try:
            subprocess.run(
                ["rsync", "-az", str(temp_path), f"{transport.host}:{remote_temp}"],
                capture_output=True,
            )
            transport.run_remote_command(
                f"cd {transport.remote_path} && "
                ".venv/bin/python -c 'import json; "
                "from src.core.data.topics import delete_topic; "
                "data = json.load(open(\"{remote_temp}\")); "
                "items = data.items() if isinstance(data, dict) else []; "
                "[delete_topic(tid, delete_children=(bool(meta.get(\"delete_children\", False)) if isinstance(meta, dict) else False)) "
